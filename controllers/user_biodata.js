...
module.exports = {
    createB: async (req, res, next) => {
        try {
            ...
            const userB = await user_game_biodata.findOne({ where: { username: username }});
            if (userB){
            ...
            const userBi = await user_game_biodata.create({
                user_id,
                username,
                rank,
                negara
    ...
    updateB: async (req, res, next) => {
        try {
            ...
            const userup = await user_game_biodata.findone({ where : {username: username }});
            if (userup){
